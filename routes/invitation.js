...
router.post('/invite', [
  ...
  body('invitee_email').isEmail().withMessage('유효하지 않은 이메일 주소입니다.'),
], async (req, res) => {
  ...
  try {
    ...
    if (memberCheck.length === 0) {
      ...
      return res.status(403).json({ error: '해당 모임통장에 대한 초대 권한이 없습니다.' });
    }
    ...
    if (inviteeRows.length === 0) {
      await connection.end();
      return res.status(404).json({ error: '초대할 사용자를 찾을 수 없습니다.' });
    }
    ...
    await connection.execute(
      ...
      [group_pk, group_pk, invitee_pk, 'member', 'pending', invite_code, expiration_date]
    );
...
router.get('/join/:invite_code',  isAuthenticated, async (req, res) => {
  ...
  try {
    ...
    if (invitation.client_pk !== client_pk) {
      await connection.end();
      return res.status(403).json({ error: '초대된 사용자만 참여할 수 있습니다.' });
    }
